...
ImGuiStyle& DialogStyle::load(const std::string& styleFilePath) const {
...
ImGuiStyle& DialogStyle::loadDefault() const {
  return this->load(Settings::Instance()->appFolder() + "/KuplungStyleDefault.style");
}

ImGuiStyle& DialogStyle::loadCurrent() const {
...
std::vector<std::string> DialogStyle::splitString(const std::string& s, const std::regex& delimiter) const {
...
ImVec4 DialogStyle::tov4(const std::string& opValue) const {
...
ImVec2 DialogStyle::tov2(const std::string& opValue) const {
  std::vector<std::string> values = this->splitString(opValue, std::regex(","));
  if (values.size() == 2)
